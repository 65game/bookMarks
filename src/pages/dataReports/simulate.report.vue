...
<template>
	<div class="box" style="min-width: 1000px;">
		...
		<el-row class="h-scoreSet-btnBox" style="margin-left: 16px; text-align: center;">
			<el-button type="primary" icon="close" @click="gotoBack">关闭</el-button>
		</el-row>
...
<script>
	...
	export default{
		...
		methods:{
			...
			gotoBack(){
				//this.$router.push({ path: '/customerList/simulate'});
				this.$bus.$emit('H_simulateShow');
			},
			...
			topDataRender(params){
				this.topData = {
					...
					suggestAmount:params.suggestAmount ||'— —' //建议额度
				}
			},
		...
		created(){
			this.$bus.$off('H_simulate_echarts');
			this.$bus.$on('H_simulate_echarts',(params)=>{ //分数，等级
				this.chartInit(params)
			});
			this.$bus.$off('H_simulate_topData');
			this.$bus.$on('H_simulate_topData',(params)=>{  //图形右侧信息
				this.topDataRender(params)
			});

			this.$bus.$off('H_simulate_customer');
			this.$bus.$on('H_simulate_customer',(params)=>{  //客户分析报表填充数据
				this.customerRender(params)
			});

			this.$bus.$off('H_simulate_idVerification');
			this.$bus.$on('H_simulate_idVerification',(params)=>{  //身份验证填充数据
				this.idVerificationRender(params)
			});

			this.$bus.$off('H_simulate_preLoan');
			this.$bus.$on('H_simulate_preLoan',(params)=>{  //贷前风险分析填充数据
				this.preLoanRender(params)
			});

			this.$bus.$off('H_simulate_hitRule');
			this.$bus.$on('H_simulate_hitRule',(params)=>{  //命中规则填充数据
				this.hitRuleRender(params)
			});

			this.$bus.$off('H_simulate_hitRating');
			this.$bus.$on('H_simulate_hitRating',(params)=>{  //命中等级填充数据
				this.hitRatingRender(params)
			});
		...
		mounted:function(){
			var _this=this;
			//初始化echarts实例
			//_this.chartInit({point:900,level:'AA'});
			_this.chartInit();
		}
